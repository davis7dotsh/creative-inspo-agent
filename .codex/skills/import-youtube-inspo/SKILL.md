...
3. Resolve the URLs with explicit JSON or JSONL `oytc` output, expand channels and playlists fully, deduplicate IDs, then fetch canonical video details in batches. Read each canonical duration, discard every video with `durationSeconds < 180`, and continue only with eligible videos. Apply requested newest-N or per-channel caps to eligible videos, fetching additional uploads when needed so excluded Shorts do not consume the requested quota.
4. Tell the user the resolved, excluded-Short, and eligible counts. If no eligible videos remain, report that outcome without creating staging files. Otherwise run Creative Agent `status`, create a task-specific directory inside `data.storage.staging`, and download each eligible video's best available thumbnail there while retaining every source thumbnail URL in the prepared record. Never download, describe, embed, stage, or upsert an excluded video's assets, and never use an unrelated system temporary directory.
...
7. Inspect `schema videos-upsert`, verify every prepared item has `durationSeconds >= 180`, build the prepared batch, and pass it through JSON stdin to `videos upsert` exactly once. The CLI rejects under-180-second videos as a backstop. Do not retry with a reduced batch after failure.
