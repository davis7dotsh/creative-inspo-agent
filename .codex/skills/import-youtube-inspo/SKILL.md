...
3. Resolve the URLs with explicit JSON or JSONL `oytc` output, expand channels and playlists fully, deduplicate video IDs, then fetch canonical video details in batches. Deduplicate the resulting channel IDs and resolve their current snippets with `oytc channel get <ID>... --parts snippet --format json`; retain every `snippet.thumbnails` URL and dimension.
4. Tell the user how many unique videos and channels were resolved. Run Creative Agent `status`, create a task-specific directory inside `data.storage.staging`, and download each best available video thumbnail and highest-resolution channel avatar there while retaining every source URL in the corresponding prepared record. Never use an unrelated system temporary directory.
...
7. Inspect `schema videos-upsert`, build one prepared batch containing `videos` and deduplicated `channels`, and pass it through JSON stdin to `videos upsert` exactly once. Each channel must contain its stable YouTube channel ID, current title, every avatar variant, and the staged highest-resolution avatar path. Do not retry with a reduced batch after failure.
