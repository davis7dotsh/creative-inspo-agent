...
- Treat every video under 180 seconds as a Short and exclude it completely. A video of exactly 180 seconds is eligible.
- Treat channel and playlist imports as one-time snapshots. Deduplicate videos by YouTube video ID.
- Deduplicate channels by YouTube channel ID. Resolve current channel snippets on every import so
  repeated imports refresh channel titles and avatars.
...
3. Resolve the URLs with explicit JSON or JSONL `oytc` output, expand channels and playlists fully, deduplicate video IDs, then fetch canonical video details in batches. Read each canonical duration, discard every video with `durationSeconds < 180`, and continue only with eligible videos. Apply requested newest-N or per-channel caps to eligible videos, fetching additional uploads when needed so excluded Shorts do not consume the requested quota. Deduplicate the channel IDs of the eligible videos and resolve their current snippets with `oytc channel get <ID>... --parts snippet --format json`; retain every `snippet.thumbnails` URL and dimension.
4. Tell the user the resolved, excluded-Short, and eligible video counts plus the unique channel count. If no eligible videos remain, report that outcome without creating staging files. Otherwise run Creative Agent `status`, create a task-specific directory inside `data.storage.staging`, and download each eligible video's best available thumbnail and each channel's highest-resolution avatar there while retaining every source URL in the corresponding prepared record. Never download, describe, embed, stage, or upsert an excluded video's assets, and never use an unrelated system temporary directory.
...
7. Inspect `schema videos-upsert`, verify every prepared item has `durationSeconds >= 180`, build one prepared batch containing `videos` and deduplicated `channels`, and pass it through JSON stdin to `videos upsert` exactly once. The CLI rejects under-180-second videos as a backstop. Each channel must contain its stable YouTube channel ID, current title, every avatar variant, and the staged highest-resolution avatar path. Do not retry with a reduced batch after failure.
8. Report the applied video count and enriched channel count in a concise table. On failure, state the failing phase and concrete error; never imply that anything was imported. Clean up only the task-specific staging directory after the terminal outcome.
